...
@MainActor
final class PipelineRunner: ObservableObject, Identifiable {
    ...
    func lines(for runID: UUID) async -> [LogLine] {
        let pid = config.id
    ...
    private func loadPipelineFile(in dir: String) async -> MaconPipeline? {
        ...
        guard let data = json.data(using: .utf8),
              let pipeline = try? JSONDecoder().decode(MaconPipeline.self, from: data),
              pipeline.hasContent else {
